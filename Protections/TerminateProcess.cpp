...
#define PROCNAME ( "TerminateProcess" )
...
void terminateProcessReset( void )
{
	g_applied = false;
}


void terminateProcessApply( bool protect )
{
	if( protect )
	{
		if( g_applied ) return;
		...
		// If dwProcessId is protected
		DWORD pid = GetCurrentProcessId();
		wsprintf( cmd, L"cmp dword ptr[esp + 0x04], 0x%08X", pid );
		ASSEMBLE_AND_STEP( cmd, localEip );
		ASSEMBLER_SET_LABEL( "processProtectedPatch", localEip );
		...
		ASSEMBLER_SET_LABEL( "processProtectedLabel", localEip );
		ASSEMBLE_AND_STEP( L"mov eax, 0x01", localEip );
		ASSEMBLE_AND_STEP( L"ret 0x08", localEip );

		// Resolve addresses which only known at the end
		patchEip = ASSEMBLER_GET_LABEL( "processProtectedPatch" );
		labelEip = ASSEMBLER_GET_LABEL( "processProtectedLabel" );

...
#define PROCNAME ( "FindWindowW" )
#define HOOK_SIZE ( 0x200 )
#define CLASS_NAME ( L"OLLYDBG" )

static bool g_applied = false;
static sHook g_hook = { 0 };


void findWindowWReset( void )
{
	g_applied = false;
}


void findWindowWApply( bool protect )
{
	if( protect )
	{
		if( g_applied ) return;
		...
		ASSEMBLE_AND_STEP( L"mov esi, dword ptr[esp + 0x0C]", localEip );
		...
		ASSEMBLE_AND_STEP( L"mov ax, word ptr[esi]", localEip );
		ASSEMBLE_AND_STEP( L"mov dx, word ptr[edi]", localEip );
		ASSEMBLE_AND_STEP( L"add esi, 2", localEip );
		ASSEMBLE_AND_STEP( L"add edi, 2", localEip );
		ASSEMBLE_AND_STEP( L"cmp ax, dx", localEip );
		ASSEMBLER_SET_LABEL( "notSameClassPatch", localEip );
		wsprintf( cmd, L"jnz 0x%08X", codeBase + localEip );
		ASSEMBLE_AND_STEP( cmd, localEip );
		ASSEMBLE_AND_STEP( L"cmp ax, 0x00", localEip );
		...
		ASSEMBLER_ADD_WSTRING_AND_STEP( CLASS_NAME, localEip );

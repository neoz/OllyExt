...
#define PROCNAME ( "NtSetDebugFilterState" )
...
void ntSetDebugFilterStateReset( void )
{
	g_applied = false;
}


void ntSetDebugFilterStateApply( bool protect )
{
	if( protect )
	{
		if( g_applied ) return;
		...
		// Set the proper NTSTATUS :)
		ASSEMBLE_AND_STEP( L"test eax, eax", localEip );
		ASSEMBLER_SET_LABEL( "notDetectedPatch", localEip );
		wsprintf( cmd, L"jnz 0x%08X", codeBase + localEip );
		ASSEMBLE_AND_STEP( cmd, localEip );
		ASSEMBLE_AND_STEP( L"mov eax, 0xC0000022", localEip );

		// Return and clean the stack
		ASSEMBLER_SET_LABEL( "notDetectedLabel", localEip );
		ASSEMBLE_AND_STEP( L"ret 0x0C", localEip );

		// Resolve addresses which only known at the end
		patchEip = ASSEMBLER_GET_LABEL( "notDetectedPatch" );
		if( patchEip )
		{
			labelEip = ASSEMBLER_GET_LABEL( "notDetectedLabel" );
			wsprintf( cmd, L"jnz 0x%08X", codeBase + labelEip );

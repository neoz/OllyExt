...
)
{
	if( IS_AT_LEAST_ONE_PROTECTION_ENABLED() )
	{
			...
			kdDebuggerEnabled )
		{
			...
			ASSEMBLE_AND_STEP( L"cmp dword ptr[esp + 0x04], 0x23", localEip );
			...
			if( kdDebuggerNotPresent )
			{
				...
				ASSEMBLE_AND_STEP( L"mov byte ptr[edx], 0x00", localEip );
			}
			...
			if( kdDebuggerEnabled )
			{
				...
				ASSEMBLE_AND_STEP( L"mov edx, [esp + 0x08]", localEip );
				ASSEMBLE_AND_STEP( L"mov byte ptr[edx + 0x01], 0x01", localEip );
			}
		}
